...
fn parse_limit_is_not_an_alias() {
    ...
    assert_eq!(Some(1), ast.limit);

    let ast = verified_query("SELECT 1 LIMIT 5");
    assert_eq!(Some(5), ast.limit);
}
...
fn parse_offset() {
    ...
    match ast.body {
        SetExpr::Select(s) => match only(s.from).relation {
            TableFactor::Derived { subquery, .. } => {
                assert_eq!(subquery.offset, Some(2));
            }
            _ => panic!("Test broke"),
        },
        _ => panic!("Test broke"),
